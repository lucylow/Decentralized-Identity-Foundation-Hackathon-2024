...
function VcGatedDapp() {
  ...
  const [vcStatusMessage, setVcStatusMessage] = useState('');
  ...
  // Mock function to verify the VC
  function verifyVc() {
    // Placeholder verification logic - replace with real verification as needed
    const isVerified = true; // assume VC verification is successful
    setVcVerified(isVerified);
    setVcStatusMessage(isVerified ? 'VC verified successfully!' : 'VC verification failed.');
  }
    ...
    <div id="vc-gated-dapp">
      ...
      <Box>
        <Container maxW={'80%'} py={4}>
          ...
          {showConnectionInfo && (
            <Box py={4}>
              {account.isConnected ? (
                ...
                <p>No account connected. Connect wallet to interact with dapp</p>
              )}
            </Box>
          )}
          <Card my={4} p={4}>
          ...
          <Box mt={6}>
            <Heading size="md">Verifiable Credential (VC) Verification</Heading>
            <Button onClick={verifyVc} mt={4}>
              Verify VC
            </Button>
            {vcStatusMessage && (
              <Text color={vcVerified ? 'green.400' : 'red.400'} mt={2}>
                {vcStatusMessage}
              </Text>
            )}
          </Box>
